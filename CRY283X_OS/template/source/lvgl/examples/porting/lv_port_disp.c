...
static void disp_init(void)
{
    ...
    lcd_init();
    //LCD_BackLight(true);
	LCD_Fill(0, 0, 240, 320, lv_color_make(0xff, 0xff, 0xff).full);
}
...
//    }
//#endif
    uint16_t xsta = area->x1;
    uint16_t xend = area->x2;
    uint16_t ysta = area->y1; 
...
//	//xs = area->x1;
//	//xe = area->x2;
//			_HW_DrawPoint(x, y, 1, 1,(uint16_t*)color_p);
//			//_HW_DrawPoint(xs, ys, xe, ye,(uint16_t*)color_p);
//            /* Put a pixel to the display. For example: */
//            /* put_px(x, y, *color_p)*/
//            color_p++;
//        }
